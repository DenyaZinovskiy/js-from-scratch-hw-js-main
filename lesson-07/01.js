...
prevButton.addEventListener("click", (e) => {
    const ourImg = document.getElementById("web-tech-image");
    let position = WEB_TECH_IMAGES.indexOf(ourImg.src);
    if (position === 0) {
        position = WEB_TECH_IMAGES.length - 1;
        ourImg.src = WEB_TECH_IMAGES[position]
    } else {
        ourImg.src = WEB_TECH_IMAGES[position - 1]
    }
})
...
nextButton.addEventListener("click", (e) => {
    const ourImg = document.getElementById("web-tech-image");
    let position = WEB_TECH_IMAGES.indexOf(ourImg.src);
    if (position === WEB_TECH_IMAGES.length - 1) {
        position = 0;
        ourImg.src = WEB_TECH_IMAGES[position]
    } else {
        ourImg.src = WEB_TECH_IMAGES[position + 1]
